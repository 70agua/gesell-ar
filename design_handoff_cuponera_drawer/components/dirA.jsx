...
// Ficha de oferta en tira horizontal — imagen a la izquierda, protagonista
function AOfertaStrip({ disc, title, partner, coins, photo }) {
  return (
    <div style={{ display: 'flex', background: '#fff', border: `1px solid ${A.line}`, borderRadius: 18, overflow: 'hidden' }}>
      <div style={{ position: 'relative', width: 260, flexShrink: 0 }}>
        <Photo kind={photo} />
        <div style={{ position: 'absolute', top: 12, right: 12, background: 'rgba(255,255,255,0.95)', borderRadius: 999, padding: '5px 10px', fontFamily: A.font, fontSize: 12, fontWeight: 600, color: A.ink, display: 'inline-flex', alignItems: 'center', gap: 4 }}>
          <span style={{ width: 12, height: 12, borderRadius: '50%', background: A.yellow, display: 'inline-block' }} />{coins}
        </div>
        <div style={{ position: 'absolute', bottom: 16, left: 18, color: '#fff', fontFamily: A.font, fontSize: 48, fontWeight: 800, letterSpacing: '-0.02em', lineHeight: 1 }}>{disc}</div>
      </div>
      <div style={{ flex: 1, padding: '22px 24px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        <div style={{ fontFamily: A.font, fontSize: 19, color: A.ink, fontWeight: 700, lineHeight: 1.25, letterSpacing: '-0.01em' }}>{title}</div>
        <div style={{ fontFamily: A.font, fontSize: 14, color: A.muted, marginTop: 5 }}>{partner}</div>
        <button style={{ marginTop: 18, alignSelf: 'flex-start', background: A.primary, color: '#fff', border: 'none', borderRadius: 12, padding: '11px 22px', fontFamily: A.font, fontSize: 14, fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: 8 }}>
          <Icon.ticket width={16} height={16} /> Añadir a cuponera
        </button>
      </div>
    </div>);
  ...
  return (
    <div style={{ background: '#fff', fontFamily: A.font, color: A.ink }}>
      ...
      <section style={{ padding: '0 56px 64px', display: 'grid', gridTemplateColumns: '1.6fr 1fr', gap: 56, alignItems: 'flex-start' }}>
        <div>
          {/* Amenities */}
          <h3 style={{ fontFamily: A.font, fontSize: 20, fontWeight: 600, color: A.ink, margin: '12px 0 16px' }}>¿Qué destaca este alojamiento?</h3>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginTop: 20 }}>
            <AOfertaStrip disc="-35%" title="Hot Sale frente al mar — 3 noches" partner="Hotel Spa Las Olas" coins={3} photo="pool" />
            <AOfertaStrip disc="2×1" title="Spa día completo, de lunes a jueves" partner="Hotel Spa Las Olas" coins={1} photo="spa" />
          </div>
        </div>
        ...
        <div style={{ position: 'sticky', top: 20 }}>
          <div style={{ background: '#fff', border: `1px solid ${A.line}`, borderRadius: 20, padding: 22, boxShadow: '0 20px 60px -30px rgba(11,16,32,0.22)' }}>
            ...
            <div style={{ marginTop: 18, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0, border: `1px solid ${A.line}`, borderRadius: 12, overflow: 'hidden' }}>
              ...
              <div style={{ padding: 12, borderTop: `1px solid ${A.line}`, gridColumn: '1 / -1' }}>
                ...
                <div style={{ display: 'flex', gap: 16, marginTop: 7, flexWrap: 'wrap' }}>
                  {[['Adultos', '2'], ['Niños', '1'], ['Bebés', '0'], ['Mascotas', '1']].map(([k, v]) =>
                  <div key={k} style={{ display: 'flex', alignItems: 'baseline', gap: 5 }}>
                      <span style={{ fontFamily: A.font, fontSize: 15, fontWeight: 700, color: A.ink }}>{v}</span>
                      <span style={{ fontFamily: A.font, fontSize: 12, color: A.muted }}>{k}</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            <button style={{ marginTop: 8, width: '100%', background: '#fff', color: A.ink, border: `1px solid ${A.line}`, borderRadius: 14, padding: '12px 0', fontFamily: A.font, fontWeight: 500, fontSize: 14, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
              <Icon.chat width={16} height={16} /> Mensajes directos
            </button>
          ...
          {/* Denunciar perfil — fuera de la caja */}
          <button style={{ marginTop: 18, width: '100%', background: 'transparent', border: 'none', color: A.muted, fontFamily: A.font, fontSize: 13, fontWeight: 500, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 7, textDecoration: 'underline', textUnderlineOffset: 3 }}>
            <svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 22V4" /><path d="M4 4h11l-1.5 4L15 12H4" /></svg>
            Denunciar un problema
          </button>
        </div>
      </section>

      {/* Ofertas exclusivas para huéspedes — tarjetas verticales (formato del resto de la web) */}
      <section style={{ padding: '64px 56px', background: A.bg, borderTop: `1px solid ${A.line}` }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 32 }}>
          <div>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontFamily: A.font, fontSize: 12, fontWeight: 600, color: A.primary, letterSpacing: '0.06em', textTransform: 'uppercase' }}>
              <Icon.bolt width={12} height={12} /> Beneficio para huéspedes
            </div>
            <h2 style={{ fontFamily: A.font, fontSize: 40, color: A.ink, margin: '12px 0 0', fontWeight: 700, letterSpacing: '-0.025em' }}>Ofertas exclusivas alojándote aquí</h2>
            <p style={{ fontFamily: A.font, fontSize: 16, color: A.muted, marginTop: 6, maxWidth: 620 }}>Reservando en el Las Olas desbloqueás estos cupones en bares y restaurantes de la zona. Los canjeás con QR durante tu estadía.</p>
          </div>
          <button style={{ background: 'transparent', border: 'none', fontFamily: A.font, fontSize: 14, fontWeight: 600, color: A.primary, display: 'inline-flex', alignItems: 'center', gap: 6, whiteSpace: 'nowrap' }}>
            Ver todas <Icon.arrowR width={16} height={16} />
          </button>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20 }}>
          <AOfertaCard disc="2×1" title="Hora feliz: tragos de autor al 2×1." partner="Cervecería Dublín" coins={1} photo="cerveza" />
          <AOfertaCard disc="-25%" title="Noche de bodegón: minutas con -25%." partner="El Viejo Hobby" coins={1} photo="rest" />
          <AOfertaCard disc="-15%" title="Churros con café de especialidad." partner="Churros El Topo" coins={1} photo="coffee" />
          <AOfertaCard disc="-20%" title="Cena de mariscos frente al mar." partner="Vista al Mar" coins={2} photo="morning" />
        </div>
      </section>

      {/* Ubicación — mapa al final */}
      <section style={{ padding: '64px 56px 80px', background: '#fff' }}>
        <div style={{ fontFamily: A.font, fontSize: 12, fontWeight: 600, color: A.primary, letterSpacing: '0.06em', textTransform: 'uppercase', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
          <Icon.pin width={12} height={12} /> Ubicación
        </div>
        <h2 style={{ fontFamily: A.font, fontSize: 40, color: A.ink, margin: '12px 0 0', fontWeight: 700, letterSpacing: '-0.025em' }}>Dónde queda</h2>
        <p style={{ fontFamily: A.font, fontSize: 16, color: A.muted, marginTop: 6 }}>Sobre la costanera, en pleno centro de Villa Gesell — a 80 metros de la playa.</p>

        <div style={{ position: 'relative', height: 460, borderRadius: 24, overflow: 'hidden', border: `1px solid ${A.line}`, marginTop: 28, background: '#E7ECE2' }}>
          <svg viewBox="0 0 1200 460" preserveAspectRatio="xMidYMid slice" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }}>
            {/* mar */}
            <path d="M900 0 L1200 0 L1200 460 L1010 460 Z" fill="#A6CBD8" />
            {/* playa */}
            <path d="M860 0 L900 0 L1010 460 L955 460 Z" fill="#EBDDBE" />
            {/* olas */}
            <g stroke="#FFFFFF" strokeWidth="3" fill="none" opacity="0.45" strokeLinecap="round">
              <path d="M1060 70 q 22 14 44 0 t 44 0" />
              <path d="M1090 170 q 22 14 44 0 t 44 0" />
              <path d="M1120 280 q 22 14 44 0 t 44 0" />
            </g>
            {/* calles verticales */}
            <g stroke="#FFFFFF" strokeWidth="12" strokeLinecap="round">
              {[110, 230, 350, 470, 590, 710, 820].map((x) => <line key={x} x1={x} y1="0" x2={x} y2="460" />)}
            </g>
            {/* calles horizontales */}
            <g stroke="#FFFFFF" strokeWidth="12" strokeLinecap="round">
              {[80, 170, 260, 350].map((y) => <line key={y} x1="0" y1={y} x2="870" y2={y} />)}
            </g>
            {/* avenida principal (costanera) */}
            <line x1="0" y1="415" x2="970" y2="415" stroke="#FFFFFF" strokeWidth="22" strokeLinecap="round" />
            {/* plazas / parques */}
            <rect x="240" y="180" width="110" height="80" rx="6" fill="#D2E0C2" />
            <rect x="590" y="80" width="120" height="90" rx="6" fill="#D2E0C2" />
          </svg>

          {/* marcador */}
          <div style={{ position: 'absolute', left: '63%', top: '60%', transform: 'translate(-50%, -100%)', textAlign: 'center' }}>
            <div style={{ background: '#fff', borderRadius: 12, padding: '9px 14px', boxShadow: '0 12px 30px -8px rgba(11,16,32,0.4)', display: 'inline-flex', alignItems: 'center', gap: 10, whiteSpace: 'nowrap' }}>
              <span style={{ width: 30, height: 30, borderRadius: 8, background: A.primary, display: 'grid', placeItems: 'center', color: '#fff', flexShrink: 0 }}><Icon.wave width={16} height={16} /></span>
              <div style={{ textAlign: 'left' }}>
                <div style={{ fontFamily: A.font, fontSize: 13, fontWeight: 700, color: A.ink }}>Hotel Spa Las Olas</div>
                <div style={{ fontFamily: A.font, fontSize: 11, color: A.muted }}>a 80m del mar</div>
              </div>
            </div>
            <div style={{ width: 2, height: 16, background: A.primary, margin: '0 auto' }} />
            <div style={{ width: 16, height: 16, borderRadius: '50%', background: A.primary, border: '3px solid #fff', margin: '-2px auto 0', boxShadow: '0 4px 10px rgba(0,0,0,0.3)' }} />
          </div>

          {/* tarjeta de dirección */}
          <div style={{ position: 'absolute', left: 20, bottom: 20, background: '#fff', borderRadius: 16, padding: '16px 18px', boxShadow: '0 16px 40px -16px rgba(11,16,32,0.3)', maxWidth: 330 }}>
            <div style={{ display: 'flex', gap: 10 }}>
              <Icon.pin width={18} height={18} style={{ color: A.primary, marginTop: 2, flexShrink: 0 }} />
              <div>
                <div style={{ fontFamily: A.font, fontSize: 14, fontWeight: 600, color: A.ink }}>Av. Costanera 1840 · Centro</div>
                <div style={{ fontFamily: A.font, fontSize: 12, color: A.muted, marginTop: 2 }}>Villa Gesell, Buenos Aires</div>
              </div>
            </div>
            <button style={{ marginTop: 14, background: A.primary, color: '#fff', border: 'none', borderRadius: 10, padding: '10px 16px', fontFamily: A.font, fontSize: 13, fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: 6 }}>
              Cómo llegar <Icon.arrowR width={14} height={14} />
            </button>
          </div>

          {/* controles de zoom */}
          <div style={{ position: 'absolute', top: 16, right: 16, display: 'flex', flexDirection: 'column', background: '#fff', borderRadius: 10, overflow: 'hidden', boxShadow: '0 6px 16px rgba(11,16,32,0.18)' }}>
            <button style={{ width: 38, height: 38, border: 'none', background: '#fff', color: A.ink, display: 'grid', placeItems: 'center', borderBottom: `1px solid ${A.line}` }}><Icon.plus width={16} height={16} /></button>
            <button style={{ width: 38, height: 38, border: 'none', background: '#fff', color: A.ink, fontSize: 20, fontWeight: 600, lineHeight: 1 }}>−</button>
          </div>
        </div>
      </section>
    </div>);
